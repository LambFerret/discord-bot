    ...
    execute: async (interaction) => {
        ...
        switch (name) {
            ...
            case _1.ButtonName.broadcast_chzzk:
                ...
                return;
            case _1.ButtonName.broadcast_afreeca:
                ...
                await interaction.update({ components: [await broadcastButtons(guildId)] });
                return;
            case _1.ButtonName.broadcast_youtube:
                ...
                await interaction.update({ components: [await broadcastButtons(guildId)] });
                return;
            case _1.ButtonName.broadcast_twitch:
                ...
                await interaction.update({ components: [await broadcastButtons(guildId)] });
                return;
            case _1.ButtonName.new_post_chzzk:
                ...
                return;
            case _1.ButtonName.new_post_afreeca:
                ...
                await interaction.update({ components: [await newPostButtons(guildId)] });
                return;
            case _1.ButtonName.new_post_youtube:
                ...
                await interaction.update({ components: [await newPostButtons(guildId)] });
                return;
            case _1.ButtonName.owner_chat_chzzk:
                ...
                return;
            case _1.ButtonName.owner_chat_afreeca:
                ...
                await interaction.update({ components: [await ownerChatButtons(guildId)] });
                return;
            case _1.ButtonName.owner_chat_youtube:
                ...
                await interaction.update({ components: [await ownerChatButtons(guildId)] });
                return;
            case _1.ButtonName.else_naver_cafe:
...
const initialButtons = () => {
    return new discord_js_1.ActionRowBuilder()
        .addComponents(makeButton(_1.ButtonName.broadcast, true), makeButton(_1.ButtonName.new_post, true), makeButton(_1.ButtonName.owner_chat, false), makeButton(_1.ButtonName.else, false));
};
const broadcastButtons = async (guildId) => {
    const info = await ServerRepository_1.default.getDetectInfo(guildId);
    return new discord_js_1.ActionRowBuilder()
        .addComponents(makeButton(_1.ButtonName.broadcast_chzzk, info.broadcastDetect.chzzk), makeButton(_1.ButtonName.broadcast_afreeca, info.broadcastDetect.afreeca), makeButton(_1.ButtonName.broadcast_youtube, info.broadcastDetect.youtube), makeButton(_1.ButtonName.broadcast_twitch, info.broadcastDetect.twitch), makeButton(_1.ButtonName.back, true, true));
};
const newPostButtons = async (guildId) => {
    const info = await ServerRepository_1.default.getDetectInfo(guildId);
    return new discord_js_1.ActionRowBuilder()
        .addComponents(makeButton(_1.ButtonName.new_post_chzzk, info.newPostDetect.chzzk), makeButton(_1.ButtonName.new_post_afreeca, info.newPostDetect.afreeca), makeButton(_1.ButtonName.new_post_youtube, info.newPostDetect.youtube), makeButton(_1.ButtonName.back, true, true));
};
const ownerChatButtons = async (guildId) => {
    const info = await ServerRepository_1.default.getDetectInfo(guildId);
    return new discord_js_1.ActionRowBuilder()
        .addComponents(makeButton(_1.ButtonName.owner_chat_chzzk, info.ownerChatDetect.chzzk), makeButton(_1.ButtonName.owner_chat_afreeca, info.ownerChatDetect.afreeca), makeButton(_1.ButtonName.owner_chat_youtube, info.ownerChatDetect.youtube), makeButton(_1.ButtonName.back, true, true));
};
const elseButtons = async (guildId) => {
    const info = await ServerRepository_1.default.getDetectInfo(guildId);
    return new discord_js_1.ActionRowBuilder()
        .addComponents(makeButton(_1.ButtonName.else_naver_cafe, info.elseDetect.naverCafe), makeButton(_1.ButtonName.back, true, true));
};
